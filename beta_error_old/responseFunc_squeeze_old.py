...
def get_beta_error(error):
	...
	print(np.mean(betax0))
	print(np.mean(betax1))	
	print(betax0)
	time.sleep(300)	
...
def plot_squeeze_comparison(error,savepath):	
	...
	fig.suptitle(r"$C_0$ = " + "{:.2e}".format(C_min_0)  + r"	$\beta_{max}$ =" + str(round(beta_max_error,2)))
...
#testing the beta_beating
tw40cm = get_twiss_rdt(0.,0.,error)[0]
...
#plot_squeeze_comparison(error,"collisionComparison_randomLocalSQ_localrQ_TESTNOBETA.pdf")
quad_strength = "0."
error = [error_l,error_component,error_strength,pattern_1,pattern_2,quad_component,quad_pattern_1,quad_strength,squeeze,twiss_pattern]
